...
func (s *ContactServiceOp) GetByEmail(email string, contact interface{}, option *RequestQueryOption) (*ResponseResource, error) {
	req := &SearchRequestPayload{
		After: 0,
		FilterGroups: []RequestFilterType{RequestFilterType{
			Filters: []RequestFilterGroup{RequestFilterGroup{
				Operator:     "EQ",
				PropertyName: "email",
				Value:        email,
			}},
		}},
		Properties: defaultContactFields,
		Limit:      1,
		Sorts:      []string{"email"},
	}
